...
<div class="condition-block customer_choice<?php echo ( $guest_booking || get_option('users_can_register') && !is_user_logged_in() || is_user_logged_in() ? ' default' : '' ); ?>" id="condition-new">
	<form action="" method="post" id="newAppointmentForm">
		...
		if ( $is_user_logged_in && ! $error_message ) {
            ...
            $msg = sprintf( _n( '
            <div style="background-color:#ff9c00; padding:5px; border:#000 solid 1px;">
            You are about to make a booking. Please ensure you have read the booking terms on the page and select the correct booking price.<br/>
            </div>
            ', 'You are about to request appointments for %s.', $total_appts, 'booked' ), '<em>' .  
            booked_get_name( $booked_current_user->ID ) . '</em>' ) . ' ' . _n( 'Please review and confirm that you would like to request the following appointment:', '
            Please review and confirm that you would like to request the following appointments:', $total_appts, 'booked' );            
			echo wpautop( $msg );
		}
		...
		if ( ! $is_user_logged_in && ! $error_message ) {
            ...
            ', 'Please confirm that you would like to request the following appointments:', $total_appts, 'booked' );
			echo wpautop( $msg );
		}
